...
test_exited_declared_pause_is_bounded_but_live_gate_surfaces() {
  ...
  while [ "$round" -le 6 ]; do
    PATH="$fakebin:$PATH" FM_FAKE_TMUX_WINDOW="$window" FM_FAKE_TMUX_CAPTURE="$capture_file" \
      ...
      FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" >> "$out" &
    pid=$!
    # This path performs liveness and authoritative-state reads before its stale
    # classification. Let the round reach that branch before deciding that it
    # absorbed, otherwise a 1.5s test reap can prevent the first durable recheck.
