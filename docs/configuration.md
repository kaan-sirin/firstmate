...
`data/` holds durable private fleet records such as the project and secondmate registries, captain preferences, optional shared captain preferences, learnings, backlog, briefs, scout reports, the canonical active-work dashboard at `dashboard.json`, and enrolled ship tasks' typed approval records at `<id>/ship-preflight.json`.
