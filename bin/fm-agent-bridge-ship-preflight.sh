...
copy_bounded_preflight_file() {
  local source=$1 target=$2
  within_preflight_limit "$source" || return 1
  dd if="$source" of="$target" bs="$MAX_BYTES" count=1 2>/dev/null || return 1
  within_preflight_limit "$source" && within_preflight_limit "$target"
}
...
valid_producer_revision() {
  within_preflight_limit "$1" || return 1
  jq -e '
...
within_preflight_limit "$HANDOFF" || die "preflight input exceeds the bounded size"
...
within_preflight_limit "$CLAIM" || die "preflight input exceeds the bounded size"

TMP=$(umask 077; mktemp "$REC_DIR/.ship-preflight.XXXXXX") || die "could not prepare preflight record"
if ! copy_bounded_preflight_file "$CLAIM" "$TMP" || ! chmod 600 "$TMP" || ! valid_private_file "$TMP"; then
  rm -f -- "$TMP"
...
if ! copy_bounded_preflight_file "$TMP" "$VALIDATION_DATA/$ID/ship-preflight.json" || ! chmod 600 "$VALIDATION_DATA/$ID/ship-preflight.json" \
...
if [ -e "$REC_DIR/ship-preflight.json" ] || [ -L "$REC_DIR/ship-preflight.json" ]; then
  ...
  within_preflight_limit "$REC_DIR/ship-preflight.json" || die "existing preflight record exceeds the bounded size"
