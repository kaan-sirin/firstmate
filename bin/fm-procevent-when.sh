...
interpreter_name() {
  case "$1" in
    sh|bash|dash|ash|ksh|mksh|zsh|fish|busybox|env|python|python[0-9]*|perl|perl[0-9]*|ruby|ruby[0-9]*|node|nodejs|lua|lua[0-9]*|php|php[0-9]*|R|Rscript|tclsh*|wish*|awk|gawk|mawk|nawk|sed|ed|ex|vi|make|gmake|bmake|find)
      return 0
      ;;
    *) return 1 ;;
  esac
...
interpreter_candidates_load() {
  ...
  INTERPRETER_CANDIDATES=(/bin/sh /bin/bash /bin/dash /usr/bin/awk /usr/bin/gawk /usr/bin/mawk /usr/bin/nawk /bin/make /usr/bin/make /usr/local/bin/make /bin/find /usr/bin/find /usr/local/bin/find)
