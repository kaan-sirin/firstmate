...
Eligibility is a firstmate judgment made BEFORE arming, because the scripts cannot classify an argv: the action must be safe, reversible, and exact. Native condition and action commands must have no arguments; use an approved direct script when a native command needs arguments.
