...
The inherited capacity value is an admission rule, so it can refuse a new spawn when the home has no free slot.
The inherited dispatch and runtime-backend values remain defaults; they must not harden `fm-spawn` to reject a deliberate runtime choice that differs from them.
